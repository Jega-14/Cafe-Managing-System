...
public class ProductServiceImpl implements ProductService {
        ...
        try{
            if (jwtFilter.isAdmin()){
                if (validateProductMap(requesMap,false)){
                    productDao.save(getProductFromMap(requesMap,false));
    ...
    private Product getProductFromMap(Map<String, String> requesMap, boolean isAdd) {
        Category category = new Category();
        category.setId(Integer.parseInt(requesMap.get("categoryId")));

        Product product = new Product();
        if (isAdd){
            product.setId(Integer.parseInt(requesMap.get("id")));
        }else {
            product.setStatus("true");
        }
        product.setCategory(category);
        product.setName(requesMap.get("name"));
        product.setDescription(requesMap.get("description"));
        product.setPrice(Integer.parseInt(requesMap.get("price")));
        return product;
    }
